                ...
                if g > 0:
                    ...
                    if(len(x_values2) >=3):
                        fig.add_trace(go.Scatter(x=x_values2, y=y_values2, mode= 'lines+markers',
                                        name="polyfem_H=2^-" + str(int(-np.log2(H))) + "_g=2^" + str(int(np.log2(g))), text="polyfem_H=2^-" + str(int(-np.log2(H))) + "_g=2^" + str(int(np.log2(g))), marker=dict(size=10)))
                    ...
                    if(len(x_values3) >=3):
                        fig.add_trace(go.Scatter(x=x_values3, y=y_values3, mode= 'lines+markers',
                                            name="polyfem_hex_H=2^-" + str(int(-np.log2(H))) + "_g=2^" + str(int(np.log2(g))), text="polyfem_hex_H=2^-" + str(int(-np.log2(H))) + "_g=2^" + str(int(np.log2(g))), marker=dict(size=10)))
                
    if x_key == "Hg":
            
        for h in thickness:
            x_values1, y_values1, name1 = accumulate_data_pesopt(data_list1, "H", y_key, "h", h)

            fig.add_trace(go.Scatter(x=x_values1, y=y_values1, mode= 'lines+markers',
                                    name="pesopt_h=2^-" + str(int(-np.log2(h))) , text="pesopt_h=2^-" + str(int(-np.log2(h))), marker=dict(size=10)))
            for i in range(1, 6):
                factorHg =  2.**(-i)
                x_values2, y_values2, name2 = accumulate_data_polyfem_Hg(data_list2, "H", y_key,"h", h, factorHg)

                if(len(x_values2) >=3):
                    fig.add_trace(go.Scatter(x=x_values2, y=y_values2, mode= 'lines+markers',
                                        name="polyfem_h=2^-" + str(int(-np.log2(h))) + "_g= H * 2^-" + str(int(-np.log2(factorHg))), text="polyfem_h=2^-" + str(int(-np.log2(h))) + "_g= H * 2^-" + str(int(-np.log2(factorHg))), marker=dict(size=10)))
    ...
    if x_key == "H":
        ...
        for h in thickness:
            ...
            for g in outofplaneRes_div:
                ...
                if(len(x_values2) >=3):
                    fig.add_trace(go.Scatter(x=x_values2, y=y_values2, mode= 'lines+markers',
                                        name="polyfem_tet_h=2^-" + str(int(-np.log2(h))) + "_g=2^" + str(int(np.log2(g))), text="polyfem_tet_h=2^-" + str(int(-np.log2(h))) + "_g=2^" + str(int(np.log2(g))), marker=dict(size=10)))
                x_values3, y_values3, name3 = accumulate_data_polyfem(data_list3, x_key, y_key,"h", h, "g", g)
        ...
        for h in thickness:
            ...
            for g in outofplaneRes_div:
                ...
                if(len(x_values2) >=1):
                    if g >0:
                            fig.add_trace(go.Scatter(x=x_values2, y=y_values2, mode= 'lines+markers',
                                                name="polyfem_h=2^-" + str(int(-np.log2(h))) + "_g=" + str(2/g), text="polyfem_h=2^-" + str(int(-np.log2(h))) + "_g=" + str(2/g), marker=dict(size=10)))
                    else:
                        fig.add_trace(go.Scatter(x=x_values2, y=y_values2, mode= 'lines+markers',
                                            name="polyfem_h=2^-" + str(int(-np.log2(h))) + "_g=1" , text="polyfem_h=2^-" + str(int(-np.log2(h))) + "_g=1" , marker=dict(size=10)))
                ...
                try:
                    ...
                    if(len(x_values3) >=1):
                        if g >0:
                            fig.add_trace(go.Scatter(x=x_values3, y=y_values3, mode= 'lines+markers',
                                                name="polyfem_hex_h=2^-" + str(int(-np.log2(h))) + "_g=" + str(2/g), text="polyfem_hex_h=2^-" + str(int(-np.log2(h))) + "_g=" + str(2/g), marker=dict(size=10)))
                        else:
                            fig.add_trace(go.Scatter(x=x_values3, y=y_values3, mode= 'lines+markers',
                                                name="polyfem_hex_h=2^-" + str(int(-np.log2(h))) + "_g=1" , text="polyfem_hex_h=2^-" + str(int(-np.log2(h))) + "_g=1" , marker=dict(size=10)))
