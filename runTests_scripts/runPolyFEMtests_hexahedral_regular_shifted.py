        ...
        for g in outofplaneRes:
            for nu in poissonRatios:
                for alpha in shift_angle:
                    ...
                    popen = subprocess.Popen("mkdir resultsPolyFEM_hexahedral/" + file3D + "_nu" + strnu + "_hexahedral_shift" + str(alpha), stdout = subprocess.PIPE, shell = True )
                    popen.wait()
                    popen = subprocess.Popen(path_to_polyfem_bin + " -j json/run_" + file3D + "_nu" + strnu + "_hexahedral_shift" + str(alpha) + ".json", stdout = subprocess.PIPE, shell = True )
                    popen.wait()
