    ...
    for h in thickness:
        for g in outofplaneRes:
            for nu in poissonRatios:
                ...
                popen = subprocess.Popen("mkdir resultsPolyFEM_prism/" + file3D + "_nu" + strnu + "_prism", stdout = subprocess.PIPE, shell = True )
                popen.wait()
                popen = subprocess.Popen(path_to_polyfem_bin + " -j json/run_" + file3D + "_nu" + strnu + "_prism.json", stdout = subprocess.PIPE, shell = True )
                popen.wait()
