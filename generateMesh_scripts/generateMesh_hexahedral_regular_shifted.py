    ...
    for h in thickness:
        for g in outofplaneRes:
            for s in shift_factor:
                file3D = "regularplate3D_h2-" + str(int(-np.log2(h))) + "_H2-" + str(int(-np.log2(H))) + "_g2+" + str(int(np.log2(g))) + "_hexahedral_shift2-" + str(int(-np.log2(s))) 
                        ...
                        for i in range(int(1 / H ) + 1):
                            for j in range(int(1 / H ) + 1 ):
                                ...
                                if k == 1 and H * j != 0 and H * j != 1 and H * i != 0 and H * i != 1:
                                    f.write(str(node_nr) + " " + str(H * j + H * s) + " " + str( H * i + H * s) + " " + str ( k * h / g) + "\n")
                                else:
