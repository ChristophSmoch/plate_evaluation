...
for H in inplaneRes:
    for h in thickness:
        for g in outofplaneRes:
            file3D = "regularplate3D_h2-" + str(int(-np.log2(h))) + "_H2-" + str(int(-np.log2(H))) + "_g2+" + str(int(np.log2(g))) + "_hexahedral_shifted" 
                    ...
                    for i in range(int(1 / H ) + 1):
                        for j in range(int(1 / H ) + 1 ):
                            ...
                            if k == 1 and H * j != 0 and H * j != 1 and H * i != 0 and H * i != 1:
                                f.write(str(node_nr) + " " + str(H * j + H * shift_factor) + " " + str( H * i + H * shift_factor ) + " " + str ( k * h / g) + "\n")
                            else:
