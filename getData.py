...
for H in inplaneRes:
    for h in thickness:
        for nu in poissonRatios:
            ...
            with open("resultsPESOPT/" + file2D + "/ConvergenceAdaptiveRef/convergenceTimeSolve.dat", "r") as f:
                for lines in f:
            ...
            for g in outofplaneRes:
                ...
                if H > 2**(-9):
                    try:
                        print("H = " + str(H))
                        file3D =  "pointplate3D_h2-" + str(int(-np.log2(h))) + "_H2-" + str(int(-np.log2(H))) + "_g2-" + str(int(-np.log2(g_real)) + 1) + "_nu" + strnu
                        with open("resultsPolyFEM/" + file3D + "/stats.json", "r") as f:
                            data = json.load(f)
                            solve_time_PolyFEM = data["time_solving"]
                        vec = getpolyfemVectors(H, h, g, nu)
