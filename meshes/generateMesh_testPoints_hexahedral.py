...
for H in inplaneRes:
    for h in thickness:
        for g in outofplaneRes:
            g_real = h * g
            file3D =  "pointplate3D_h2-" + str(int(-np.log2(h))) + "_H2-" + str(int(-np.log2(H))) + "_g2-" + str(int(-np.log2(g_real)) + 1) + "_hexahedral"
            with open(file3D + ".geo", "w" ) as f:
                ...
                for i in range(  (npts_sqrt - 1)**2 + 4):
                    # surfloop += "," + str(i+1)
                    f.write("Extrude {0,0,h/2} {Surface{" + str(i + 1) + "}; Layers{g}; Recombine;}\n")
                for i in range( (npts_sqrt - 1)**2 + 4):
                    f.write("Extrude {0,0,-h/2} {Surface{" + str(i + 1) + "}; Layers{g}; Recombine;}\n")
